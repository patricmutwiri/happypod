...
function quit(parent)
{
    ...
    if (GCompris.DownloadManager.downloadIsRunning()) {
        ...
        showMessageDialog(parent,
                qsTr("Quit?") +
                '\n' +
                qsTr("Do you really want to quit Happypod?"),
...
function loginNow(parent) {
    showMessageDialog(parent,
        qsTr("Enter Password or Quit") +
        '\n' +
        qsTr("You need to login to continue.") + '<br/>',
        qsTr("<br/> Enter Pass"), function() {
            //ask pass here
            aboutToQuit = false;
        },
        qsTr("Quit"), function() { Qt.quit(); },
        function() {
            aboutToQuit = false;
